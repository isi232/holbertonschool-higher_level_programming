...
Extends the Flask application with a /products route that reads
product data from either a JSON file or a CSV file, depending on a
'source' query parameter, and optionally filters by an 'id' query
parameter.
"""
import csv
import json
...
@app.route('/products')
def products():
    """Display product data from JSON or CSV, optionally filtered by id.

    Query Parameters:
        source (str): Either 'json' or 'csv'. Determines which file
            the product data is read from.
